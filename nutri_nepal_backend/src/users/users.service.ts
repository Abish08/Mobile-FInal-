...
export class UsersService {
  ...
  async findAll(search?: string, goal?: string, sortBy: string = 'createdAt') {
    ...
    const users = await this.userModel.find(query).select('-password').sort(sortOption);

    const today = new Date();
    today.setHours(0, 0, 0, 0);
    const newToday = await this.userModel.countDocuments({ createdAt: { $gte: today } });
  ...
  async getAdminStats() {
    ...
    const bmiDistribution = await this.userModel.aggregate([
      { $match: { height: { $exists: true, $ne: null }, weight: { $exists: true, $ne: null } } },
      {
        $addFields: {
      ...
      },
      {
        $addFields: {
          bmiCategory: {
            $cond: {
              if: { $lt: ['$bmi', 18.5] }, then: 'Underweight',
              else: {
                $cond: {
                  if: { $lt: ['$bmi', 25] }, then: 'Normal',
                  else: { $cond: { if: { $lt: ['$bmi', 30] }, then: 'Overweight', else: 'Obese' } }
                }
              }
            }
          }
