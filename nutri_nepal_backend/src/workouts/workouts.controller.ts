...
import type { WorkoutDocument } from './schemas/workout.schema';
...
export class WorkoutsController {
  ...
  testEndpoint() {
    return {
      message: 'SUCCESS! This endpoint is 100% public and has no guards.',
    };
  }
  ...
  async create(
    ...
    return {
      success: true,
      data: await this.workoutsService.create({ ...dto, userId: user._id }),
    };
  }
  ...
  async findAll(@CurrentUser() user: UserDocument) {
    return {
      success: true,
      data: await this.workoutsService.findAll(user._id.toString()),
    };
  }
  ...
  @Post('admin')
  @UseGuards(JwtAuthGuard, AdminGuard)
  async createAdminWorkout(@Body() dto: WorkoutInput) {
    return this.workoutsService.createAdminWorkout(dto);
  }

  @Put('admin/:id')
  @UseGuards(JwtAuthGuard, AdminGuard)
  async updateAdminWorkout(
    @Param('id') id: string,
    @Body() dto: UpdateQuery<WorkoutDocument>,
  ) {
  ...
  async uploadThumbnail(
    ...
    return this.workoutsService.updateThumbnail(
      id,
